...
class GreeterClient {
  public:
    ...
    // from the server.
    std::string SayHello(const std::string& user, bool is_hello_again) {
        ...
        if (is_hello_again) {
            status = stub_->SayHelloAgain(&context, request, &reply);
        } else {
...
int main(int argc, char** argv) {
    ...
    bool is_hello_again = false;
    if (argc >= 2 && argv[1] == std::string("again")) {
        is_hello_again = true;
    }

    std::string reply = greeter.SayHello(user, is_hello_again);
